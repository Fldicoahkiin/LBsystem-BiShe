...
public class ReaderInfoController {
    ...
    @ResponseBody
    public DataInfo queryReaderAll(ReaderInfo readerInfo, @RequestParam(defaultValue = "1") Integer pageNum,
     ...
     * 管理员修改读者密码
     */
    @PostMapping("/updateReaderPassword")
    @ResponseBody
    public DataInfo updateReaderPassword(@RequestBody ReaderInfo readerInfo) {
        // 这里可以添加更复杂的密码校验逻辑，例如长度、复杂度等
        if (readerInfo.getPassword() == null || readerInfo.getPassword().length() < 6) {
            return DataInfo.fail("密码长度不能少于6位");
        }
        try {
            // 注意：这里只更新密码字段
            ReaderInfo updateInfo = new ReaderInfo();
            updateInfo.setId(readerInfo.getId());
            updateInfo.setPassword(readerInfo.getPassword()); // 实际项目中密码应该加密存储
            readerInfoService.updateReaderInfoSubmit(updateInfo);
            return DataInfo.ok();
        } catch (Exception e) {
            // 记录日志等
            return DataInfo.fail("密码修改失败");
        }
    }
